...
function AddCity({user, country}:{user:User, country:string}){
    const emptyError = new URLSearchParams(location.search).get('empty');
    ...
    return (
        <div className="relative w-full h-full flex flex-col items-center justify-center bg-white shadow-lg rounded-lg px-4">
            <h2 className="text-4xl font-bold my-6 text-black uppercase text-center">Agregar Ciudad</h2>
            <form method="POST" action="/api/add-city" className="flex flex-col w-full gap-4 p-4">
                ...
                <span className="flex flex-row justify-between items-center w-full gap-4">
                    <label className="text-black" htmlFor="country">Pais</label>
                    <input className="p-2 rounded-md border bg-gray-100 text-gray-600" type="text" id="country" name="country" value={country} readOnly />
                </span>
                <span className="flex flex-row justify-between items-center w-full gap-4">
                    <label className="text-black" htmlFor="city">Nombre</label>
                    <input className="p-2 rounded-md border capitalize" type="text" id="city" name="city" placeholder="Madrid, Barcelona ..." />
                </span>
                <span className="flex flex-col">
                    ...
                    <span className="flex flex-row justify-around">
                        <a href={`/country/${country}`} className="w-32 text-red-500 text-center text-xl font-semibold border border-red-500 px-2 py-1 mt-4 rounded-lg transition-all hover:text-white hover:bg-red-500 hover:scale-105">
                            Atrás
                        </a>
            ...
            <ShowErrorToast message="Error al agregar la ciudad" paramError="empty" />
            <ShowErrorToast message="Ha habido un error" paramError="error" />
        </div>
    )
}
