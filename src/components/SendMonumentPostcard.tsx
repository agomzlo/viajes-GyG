...
function SendMonumentPostcard({user, users, city, country, monument}: {user: User, users: User[], city: City, country: string, monument: Monument}) {
    ...
    return (
        <div className="relative w-full h-full flex flex-col items-center justify-center bg-white shadow-lg rounded-lg px-2">
            ...
            <form method="POST" action="/api/send-monument-postcard" className="flex flex-col w-full gap-4 p-4">
                ...
                <span className="flex flex-row justify-between items-center w-full gap-4">
                    <label className="text-black" htmlFor="point">Punto</label>
                    <input className="p-2 rounded-md border bg-gray-100 text-gray-600" type="text" id="point" name="point" value={monument.name} readOnly />
                </span>
                <span className="flex flex-row justify-between items-center w-full gap-4">
                ...
                <span className="flex flex-col">
                    ...
                    <span className="flex flex-row justify-around">
                        <a href={`/country/${encodeURI(country)}/city/${encodeURI(city.name)}/monument/${encodeURI(monument.name)}`} className="w-32 text-red-500 text-center text-xl font-semibold border border-red-500 px-2 py-1 mt-4 rounded-lg transition-all hover:text-white hover:bg-red-500 hover:scale-105">
                            Atrás
                        </a>
...
export default SendMonumentPostcard;