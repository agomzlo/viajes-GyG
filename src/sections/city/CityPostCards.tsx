import type { City, PostCard, User } from "@/lib/types";

function CityPostCards({country, currentCity, user, postcards}: {country: string, currentCity: City, user: User, postcards: PostCard[]}){
    ...
    return (
        <section>
            <h2 className="font-bold text-2xl md:text-3xl mt-4">Postales de {currentCity.name}:</h2>
                                ...
                                <picture className="relative w-full h-full aspect-video">
                                    ...
                                    <div className="absolute inset-0 grid place-content-center border border-black m-5 uppercase">
                                        <span className="flex flex-col items-end scale-75">
                                            <p className="text-black text-6xl sm:text-8xl [text-shadow:1px_1px_white]">{ currentCity.name }</p>
                                        </span>
                                    </div>
                                </picture>
                            </a>
                        ...
                        receivedPostcard ? (
                            ...
                            >
                                <picture className="relative w-full h-full aspect-video transition-all hover:scale-105">
                                    ...
                                    <div className="absolute inset-0 grid place-content-center border border-black m-5 uppercase">
                                        <span className="flex flex-col items-end scale-75">
                                            <p className="text-black/50 text-6xl sm:text-8xl">{ currentCity.name }</p>
                                        </span>
                                    </div>
                                </picture>
                            </a>
